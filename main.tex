...
\begin{frame}
	\frametitle{Highly underconstrained Problem!}
	
	\begin{itemize}
		\item mixed signals 
		\item a variety of partly observed biochemical processes
		\item irregular data acquisition patterns
		\item Measurements on pixels of 10-60m ground resolution 
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\visible<1->{Learn the vegetation model}
	\visible<2->{from globally available satellite data $\M{x}$}
	
	\visible<3>{reframing as \textbf{supervised classification} of vegetation $\V{y} = (y_\text{corn},y_\text{cereals},\dots)^T$}
	...
	\begin{tikzpicture}[node distance=0em]
		...
		\visible<2->{\node[right=of f](x){$(\M{X})$};}
		
%		
%		\visible<4->{\node[below left=of y, text width=5cm, font=\normalsize]{
%		
...
%		}; } 
%		
%		\node[below right=of x, text width=3cm]{
%%			$
%%			\V{x}_t = 
%%			\begin{pmatrix}
%%			\rho_{\text{blue}_t} \\
%%			\rho_{\text{green}_t} \\
%%			\dots \\
%%			\rho_{\text{infrared}_t} \\
%%			\end{pmatrix}$};
%			satellite data available at large scale
%
%		};
	\end{tikzpicture}
}
...
%	\textbf{Intuition:}
	...
	\Large
	A \textbf{discriminative classification model} must \textbf{internalize} a \textbf{learned model for the vegetation}
...
\column{.5\textwidth}
similar to Vegetation Model of Cereals (Large et al., 1954)
%\centering 
\vspace{1em}
...
\small Large, E. C. (1954). Growth stages in cereals illustration of the Feekes scale. Plant pathology, 3(4), 128-129.
...
\visible<2>{\node[left= of equals, font=\huge](probas){fallow (no crop)};}
...
\begin{frame}
	...
	\begin{columns}
		\column{.5\textwidth}
			\only<1>{
			\includegraphics[width=.9\textwidth]{images/map/europe}
			NUTS-2 division of {\color{tumorange} Brittany (Breizh)} in Europe 
			}
			\only<2>{
				Field Parcels
				\begin{tabular}{lrr}
